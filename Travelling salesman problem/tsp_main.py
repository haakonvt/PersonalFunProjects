...
def HillClimberRandInit(dist_data,route,max_iter,rand_inits=100):
    ...
    while count < rand_inits:
        ...
        new_path, new_route,iterations = HillClimber(dist_data,iter_route,max_iter,True)
        if new_path < best_overall_path:
            best_overall_path     = new_path
            best_overall_route[:] = new_route[:]
            print "Better path:   %.2f \t" %best_overall_path, np.int_(best_overall_route) \
            ,"run:",count,", iter:", iterations
...
def MetroRandInit(dist_data,route,two_or_three,max_iter,runs):
    ...
    while count < runs:
        ...
        new_path,new_route,iterations = MetropolisFunction(dist_data,new_route,max_iter,True)
        if new_path < best_path:
            best_path = new_path
            best_route[:] = new_route[:]
            print "Better path:   %.2f \t" %best_path, np.int_(best_route)\
...
def SimulatedAnnealing(dist_data,route,iterations):
    ...
    print "  NB: Not finished"
    print "------------------------------------------------"
    print "New start path: %.2f \t" %ComputeTotalDist(dist_data,new_route), np.int_(new_route)
    from math import exp
    route_array_to_plot = []
    p0 = 0.5; p1 = 0.01
    path0 = 31500. # Average length of random 24-city path
    path1 = 12000. # Maybe less than global optimum
    ...
    for count in range(1,iterations+1):
        ...
        if new_path < cur_path:
            ...
            T  = 0.00856474*exp(0.00012911*new_path)/count**(1./2)
...
def MetroRandInitEvo(dist_data,route,max_iter,runs):
    ...
    while count < runs:
        ...
        while iterations < max_iter:
            print iterations
            rand_iter     = np.random.randint(10,150) # Number of iterations with Two/Three pass
            iterations   += rand_iter
            random_number = np.random.random_sample()
            new_route[:]  = best_route[:]
            best_path     = ComputeTotalDist(dist_data,best_route)
            if   random_number < 0:
                new_path,new_route = TwoPassMetropolis(dist_data,  new_route,rand_iter,True)
            elif random_number < 0:
                new_path,new_route = ThreePassMetropolis(dist_data,new_route,rand_iter,True)
            else:
                counter = 0
                while True:
                    counter += 1
...
def GeneticAlgo2Pass(dist_data,route,pool_size=20,generations=50,no_print=False):
    n = 24
    best_path = 1E100; glob_opt_reached = False
    mutation_param = int(n/2); mutation_list = np.zeros(pool_size)
    new_route = np.zeros(n); new_route[:] = route[:]
    if len(route) < n:
        print "This works only with 24 cities. Exiting..."
        sys.exit(0)
    ...
    for gen in range(generations):
        for current_route,i in zip(pool,range(pool_size)):
            ...
            if False:#i < pool_size/2:
                new_path,new_route,TPM_iter = TwoPassMetropolis(dist_data,new_route,3500,True)
            else:
                new_path, new_route,TPM_iter = HillClimber(dist_data,new_route,40,True)
            pool[i,:]     = new_route
        ...
        if pool_paths[i_best] < (best_path - 1E-5):
            ...
            if not no_print:
                print "Average path length %.2f" %np.mean(pool_paths),", best path %.2f" %(min(pool_paths)), ", gen",gen,", mut. param. of survivor:",mutation_list[i_best]
...
if __name__ == '__main__':
    ...
    pool_size   = 20; max_generations = 200
    print "Pool size, generations needed, found global optimum T/F"
    for i in range(10):
        gen,glob_opt_reached = GeneticAlgo2Pass(dist_data,route,pool_size,max_generations,no_print=True)
